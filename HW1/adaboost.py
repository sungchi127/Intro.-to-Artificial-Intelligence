    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        Selects the best weak classifier for the given weights.
          Parameters:
            ...
            features: A numpy array of HaarFeature class.
            might assigned to the ith training sample.
          Returns:
            bestClf: The best WeakClassifier Class
            bestError: The error of the best classifer
        """
        # Begin your code (Part 2)
        bestError=1.0;pos=0.0;nes=0.0;pot=0.0;net=0.0;tmp=0.0
        featuremap=[]
        ...
        for i in range(len(features)):
            ...
            for j in range(len(featureVals[i])):
                featuremap.append((featureVals[i][j],labels[j],weights[j]))
                featuremap.sort(key = lambda x: x[0])
            for j in range(len(featureVals[i])): 
                threshold=featuremap[j][0]
                if j==0:
                    continue

                if featuremap[j-1][1]==1:
                    pos+=featuremap[j-1][2]
                else:
                    nes+=featuremap[j-1][2]
                tmp=min(pos+(net-nes),nes+(pot-pos))
                if bestError > tmp/sum:
                    bestError=tmp/sum
                    if tmp==nes+(pot-pos):
                        bestClf=WeakClassifier(features[i],threshold,1)
                    else :
                        bestClf=WeakClassifier(features[i],threshold,-1)
